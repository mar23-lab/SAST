...
create_configuration() {
    ...
    cp "$sast_dir/ci-config.yaml" "${PROJECT_NAME}-config.yaml" 2>/dev/null || {
...
EOF
    ...
    log_success "Configuration created: ${PROJECT_NAME}-config.yaml"
}
...
env:
  CONFIG_FILE: \${PROJECT_NAME:-project}-config.yaml
...
setup_email_notifications() {
    ...
    if [[ -f "$email_wizard" ]]; then
        ...
        cat >> "\${PROJECT_NAME:-project}-config.yaml" << 'EOF'
...
notifications:
  email:
    ...
    triggers:
      ...
      scan_failed: true
EOF
        log_info "Basic email configuration template added to \${PROJECT_NAME:-project}-config.yaml"
    fi
}
...
test_installation() {
    ...
    if [[ -f "\${PROJECT_NAME:-project}-config.yaml" ]] && yq eval '.project.name' "\${PROJECT_NAME:-project}-config.yaml" >/dev/null 2>&1; then
...
show_completion_summary() {
    ...
    echo "- Configuration: \${PROJECT_NAME:-project}-config.yaml"
